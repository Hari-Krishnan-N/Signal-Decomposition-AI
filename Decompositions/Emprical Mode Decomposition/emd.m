...
%--------------------------------------------------------------------------
function [y,t,td,isTT,opt] = parseAndValidateInputs(x, varargin)
% input type checking
...
if coder.internal.isConst(MaxNumIMF)
    coder.varsize('temp',[MaxNumIMF 1],[1 0]);
end
temp = zeros(MaxNumIMF,1,ArrayType);
IMFs = repmat(temp',N,1);
info.NumIMF = temp;
info.NumExtrema = temp;
info.NumZerocrossing = temp;
info.NumSifting = temp;
info.MeanEnvelopeEnergy = temp;
info.RelativeTolerance = temp;
...
while(i<MaxNumIMF)
    ...
    % record information
    [peaksIdx, bottomsIdx] = localFindExtramaIdx(rsigL);
    numZerocrossing = nnz(diff(sign(rsigL)));
    info.NumIMF(i+1) = i+1;
